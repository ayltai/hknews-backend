...
public final class WenWeiPoParser extends Parser {
    ...
    WenWeiPoParser(@NonNull @lombok.NonNull final ApiServiceFactory apiServiceFactory, @NonNull @lombok.NonNull final SourceRepository sourceRepository) {
    ...
    public Collection<Item> getItems(@NonNull @lombok.NonNull final Category category) {
        ...
        for (final String url : category.getUrls()) {
            try {
                final String[] html = StringUtils.substringsBetween(this.apiServiceFactory.create().getHtml(url).execute().body(), "<div class=\"content-art-box\">", "</article>");
